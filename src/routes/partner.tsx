...
import { submitPartnershipInquiry } from "@/functions/partner";
  ...
  head: () => ({
    meta: [
      { title: "Partner With Us — Tare Zameen Foundation" },
      {
        name: "description",
        content:
          "Housing societies, RWAs and corporates: adopt drives, track impact, and receive verified certificates.",
      },
      { property: "og:title", content: "Partner With Tare Zameen Foundation" },
      {
        property: "og:description",
        content: "Society, RWA and corporate partnerships built on radical transparency.",
      },
...
function PartnerPage() {
  ...
  const submitInquiryFn = useServerFn(submitPartnershipInquiry);
  ...
  return (
    <>
      ...
      <main className="mx-auto min-h-dvh max-w-[1000px] px-6 pb-20 pt-32 lg:px-10">
        ...
        >
          <p className="mt-6 text-[11px] font-semibold uppercase tracking-[0.28em] text-gold">
            Stronger Together
          </p>
          ...
          <p className="mt-3 max-w-xl text-[15px] text-muted-foreground">
            Adopt drives, receive real-time impact tracking, and automated impact & CSR compliant
            reporting.
          </p>
          ...
          <form
            onSubmit={submit}
            className="mt-6 grid gap-4 rounded-2xl border border-border bg-white p-6 shadow-soft sm:grid-cols-2"
          >
            <input
              placeholder={`${TABS.find((t) => t.id === tab)?.label} name*`}
              value={form.org}
              onChange={(e) => setForm({ ...form, org: e.target.value })}
              className="rounded-lg border border-border px-3 py-2.5 text-sm outline-none focus:border-brand focus:ring-2 focus:ring-brand/25 sm:col-span-2"
            />
            <input
              placeholder="Contact name"
              value={form.name}
              onChange={(e) => setForm({ ...form, name: e.target.value })}
              className="rounded-lg border border-border px-3 py-2.5 text-sm outline-none focus:border-brand focus:ring-2 focus:ring-brand/25"
            />
            <input
              placeholder="Email*"
              type="email"
            ...
            <button
              type="submit"
              disabled={submitting}
              className="grad-brand rounded-xl px-5 py-3 text-sm font-semibold text-white shadow-glow transition hover:scale-[1.02] disabled:opacity-60 disabled:hover:scale-100 sm:col-span-2"
            >
